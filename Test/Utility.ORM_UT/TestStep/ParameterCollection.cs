...
namespace Utility.ORM_UT.TestStep
{
    ...
    public sealed class ParameterCollection
    {
        ...
        {
            ScenarioContext.Current.Add("Condition", new StubCondition(pi_sCondition, null));
        }

        [Given(@"自訂二組篩選條件式 (.*) 以及 (.*)")]
        public void Given自訂篩選條件式A及B(string pi_sCondition1, string pi_sCondition2)
        {
            ScenarioContext.Current.Add("Condition1", new StubCondition(pi_sCondition1, null));
            ScenarioContext.Current.Add("Condition2", new StubCondition(pi_sCondition2, null));
        ...
        public void Given重覆設定參數集的條件式(string pi_sCondition)
        {
            ...
            objParameters.SetCondition(new StubCondition(pi_sCondition, null));
